// $Id:$
/*
 * fecha_check.cpp - Comprobaciones estáticas de código sobre la clase Fecha
 * de P0 y P1
 * ©2015-2019 Pedro Delgado, alumnos colaboradores y profesores de POO
 */
...
int main(int argc, const char **argv){
  ...
  if(c.findClass({"Fecha"})) {
    ...
    params = {{"?"}, {"?"}, {"?"}, {"?"} };
    c.functionWithReferencedFunction({"operator!=", "operator>",
    ...
    c.fileIncludedHeader(headerName, {"fecha.cpp", "fecha.hpp"},
	       "Revisa de dónde son tomadas las funciones ssprintf y sscanf.");
